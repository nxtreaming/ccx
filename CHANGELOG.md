...
- **Codex 记忆层 `/v1/alpha/*` 透传端点** - Codex token budget 记忆系统的 `history.*`/`notes.*` namespace 工具后端直发 `POST {base}/alpha/history|notes/v2/*`，指向 CCX 时打到 `/v1/alpha/*`。新增 `handlers/alpha/` 透传处理器（粘 Responses 渠道池）：body 原样转发、`x-openai-encrypted-tool-arguments` 等非标准头经 `PrepareUpstreamHeaders` 自动透传、认证替换为渠道 Key。会话粘性三级——X-Channel pin > 推理亲和上下文桶扫描（`scheduler.PreferredChannelNameForUserSweep`，解决推理按 ContextRequirement 分桶写亲和/记忆请求无桶读不到的 key 错位）> `SelectChannel` 无桶亲和自粘（`context.session_id` 作 userID），成功后写回亲和保证同会话记忆请求固定渠道。指标刻意隔离：渠道日志用独立 identity（serviceType 标签 "Alpha"）且不写 RecordSuccess/RecordFailure/MarkKeyAsFailed——中转站普遍未实现 alpha 端点，失败并入推理指标会把渠道打成不健康污染路由。上游 404 如实透传（候选耗尽回写最后上游响应而非误导性 503），Codex 客户端自行降级为无记忆模式。
- **Codex `encrypted_function_args` 全链路透传与计数** - Codex namespace 工具的 `function_call` 可携带分段加密参数（数组形态）。`types.ResponsesItem` 新增 `EncryptedFunctionArgs` 字段并补齐请求解析（`responsesItemFromMap`）与响应侧枚举（`responses_passthrough.go`）；Responses 直通的请求侧原始 map 转发天然携带。转换到 Chat/Claude/Gemini 时密文无对应物继续丢弃，但空 `arguments` 补 `"{}"` 占位（`resolveFunctionCallItem`，防 DeepSeek 类上游拒收空串）；`token_counter` 对密文分段按保守估算法逐段计数（高熵密文 ~1.5 字符/token）；日志截断白名单加入该字段。
- **统一客户端预算提醒剥离机制（跨模型改写后剔除 tokens-left 失真数字）** - 新增 `providers/client_budget_reminders.go` 统一剥离语义：CC 的 `<total_tokens>N tokens left</total_tokens>` 与 Codex 的 "You have N tokens left in this context window." 按客户端认知窗口核算，模型一旦改写即失真，需剥离。①联邦/溢出跨模型执行改写后（`upstream_failover.go`）按请求协议剥离并同步 RestoreRequestBody/requestBodyBytes，兜住直通执行协议缺口（转换路径后续再剔一次，幂等）。②Claude passthrough 模型重定向命中时（`claude.go` redirectModelInBody）剥离 system 内预算提醒，未命中 data 零改动保缓存。③Responses passthrough 命中映射（`RedirectModelWithMatch` 判定实际换模型）才剔、未命中保留；跨协议转换无条件剔（`responses.go` 两分支）。④`openai.go` 将预算提醒正则从 claudeCodeSystemPatterns 拆分到 ccBudgetReminderPatterns，isClaudeCodeSystemHeader 经 isCCBudgetReminderText 合并判定，转换路径行为不变。新增联邦剥离测试（protocol_federation_execution_test.go）与 Responses 分支语义矩阵测试（matrix_responses_test.go）。
...
- **质量档校准 v2：固定版本化阈值 + 证据等级统一口径** - 重构模型基础档与 (模型×思考强度) 档位的评定体系。①生产阈值固定版本化（`fixed-direct-calibration-v2-2026-09-04`，premium/high/normal = 69.95/59.15/44.25），由离线校准从 medium/default 直测分布推导、锚定测试验证，不再随注册表刷新动态重算——2026-09-04 hy4-preview 凭 taskCount=5 小样本端点插值 76.1 曾把动态 premiumMin 从 60.70 推到 74.45，全榜档位随数据抓取重排。②`CalibrationResult{Score, Class, MeasuredEffort}` 结构化替代 singleEffortOnly 布尔：direct（medium 直测，唯一可证明 premium）/ interpolated（跨 medium 插值）/ deflated（全局比率折算，含旧 singleEffortOnly 语义，且同侧多档折算从不封顶收紧为封顶）/ calibrated（AA 校准），估计类证据一律封顶 high。③新增 `EffortAwareQualityTier(model, effort)` 统一 (模型×effort) 评档：该档可靠直测 > 同模型曲线插值（封顶 high）> 回落基础档（effort 低于 medium 时按全局比率保守下调）。④删除"任一高档实测达标即绕过模型级过滤"的 effort 旁路（`effortLevelQualityAdmission`）：advisor MinQualityTier 与 capability floor 的豁免改为请求实际思考等级感知——未 pin effort 时不再凭 max 档实测分放行 medium 请求（luna medium 19.6 凭 max 69.6 进 high 候选的档位错配修复），pin 了 effort 且该组合达标才豁免。⑤JS 图表阈值同步（`visualization.mjs`/`generate-benchmark-chart.mjs`，动态聚类移除）。v2 阈值下档位快照：premium={gpt-6-astra, gemini-3.8-flash}（hy4-preview 插值 76.1 封顶 high），opus-5/fable/sol/grok-4.6/gemini-3.7 为 high，kimi-k3/gpt-5.5/glm-5.3/opus-4-8 为 normal，sonnet-5 等低段模型 low；回放测试锁定（`TestModelProfileQualityTierRegistryReplay`），锚定测试对 premium singleton 等分布形态剧变报警。
...
- **拉黑按 key 跨协议级联（#8）** - `BlacklistKeyWithRecoverAt` 抽出 `applyKeyBlacklistLocked` 单渠道拉黑，随后 `siblingUpstreamsForKeyLocked` 级联拉黑同 `AccountUID`/`LogicalChannelUID` 下持有相同明文 key 的其它协议渠道（单次落盘，各自发 `key_blacklisted` 事件）；不同账号的同名 key 不误伤。修复"拉黑 chat 渠道 key 不影响同账号 claude/codex"的凭证状态隔离缺陷。
- **火山 L1 探针透传网络配置** - `internal/upstreamprobe` 新增 `ProbeOptions`，`ProbeVolcenginePlan`/`VolcenginePlanL1Probe`/`postJSONProbe` 透传 `proxyURL`/`customHeaders`/`insecureSkipVerify`；`main.go` healthCheckL1Fetcher 与 autopilot verify 接线；`docs/specs/logical-channel.md` 新增 §17 同物理端点多协议引用的运行时一致性。删除错误的 `volcengine-gemini` 渠道（火山 coding-plan 是 OpenAI/Anthropic 协议，不应注册为 Gemini 入口）。
...
- **渠道兼容性开关统一自学习** - 六项兼容性开关（`NormalizeNonstandardChatRoles`/`StripImageGenerationTool`/`CodexNativeToolPassthrough`/`StripEmptyTextBlocks`/`PassbackReasoningContent`/`PassbackThinkingBlocks`）从「用户手工勾选的静态布尔」彻底收归「运行时按渠道-Key-模型自动学习并持久化的能力事实」：字段已从 `UpstreamConfig`/`UpstreamUpdate` 整体删除，管理面板不再提供编辑入口，渠道更新接口不再接受手工写入，用户此后无需手工干预。新增 `ChannelCompatCache`（落盘 `.config/channel_compat.json`，TTL 24h，键 `channelUID:keyHash:model`）统一记忆六项兼容能力，最终生效值按 `学习结论 > 历史种子 > 静态默认` 合成。老用户配置文件里残留的历史手工值不代表可信的长期事实（可能当初就设错，也可能上游后来有了新情况），因此启动时 `migrateManualCompatSwitchesToSeeds` 从磁盘原始 JSON 读出历史值，一次性降级为渠道级、TTL 14 天的一次性低置信度种子（`CompatSeeds`）：迁移当下不改变可感知行为，但学到真实结论后立即让位，过期后也不再参与判断；种子 TTL 与学习记忆 TTL 分开存储（14 天 vs 24 小时），避免混淆。该迁移按数组下标把原始 JSON 对到当前渠道，因此必须排在 `mergeManagedProviderAccounts` 之前——后者会把同 provider 的多账号渠道合并成一条（数组变短重排），顺序颠倒会让种子写到错误渠道或随被合并渠道丢失；`mergeManagedProviderAccounts` 同时补上了合并时携带 `CompatSeeds`，避免被合并掉的渠道的历史证据丢失。`Clone()` 一并深拷贝 `LearnedCompatTraits`，避免克隆已注入结论的 upstream 时共享 map。按信号强度分三条学习路径：有硬 400 信号的走 `CompatTraitFromError` 错误驱动（识别后同 Key 立即重试，改写在下一轮注入本次请求专属的 `LearnedCompatTraits`），其中 Codex `/v1/responses` 打到不支持 `developer` role 的上游时新增 `DowngradeDeveloperRoleToSystem` 窄改写（降 `system` 而非 `user`，保留开发者指令语义）；仅能靠响应形态判断的 `passbackReasoningContent`/`stripEmptyTextBlocks` 复用渠道诊断探针，改为首次遇到该组合时 singleflight 后台自动探测一次，替代原先要用户手工点按钮；图片生成受限从「直接拉黑 (Key,模型)」改为「先学习剥离图片工具并重试，剥离后仍失败才回落拉黑」。所有识别规则采用双条件门控（正则命中 + 请求侧确实携带对应结构），且 429/5xx/超时一律不参与兼容性学习，避免把容量问题学成能力缺失。`RuntimeUpstreamForAutoManagedProvider` 连种子一并清空，自动托管渠道的兼容性完全由厂商原生默认加运行时学习决定
- **Kimi 参数硬约束迁入 model-registry 刷新链路** - Kimi K3/K2.7-code/K2.6 的 temperature/top_p/n 固定值、tool_choice/thinking 约束不再是本地硬编码前缀表，改为 `UpstreamModelCapability.ParamConstraints`，随 `shared/model-registry/ccx_model_registry.json` 走 presetstore 的定期刷新链路发布；运营者核对厂商文档后更新 JSON 即可让线上实例在下一个轮询周期生效，不需要重新编译发版
- **Kimi 按量模型参数硬约束前置规避** - `ApplyKnownParamConstraints` 按厂商文档在发送前规避已知约束，无需先失败一次：`kimi-k3`/`kimi-k2.7-code` 系列/`kimi-k2.6` 的 `temperature`/`top_p`/`n`/`presence_penalty`/`frequency_penalty` 为固定值，传入即 `invalid_request_error`，故直接剥离（上游用自身默认值，语义无损）；`kimi-k2.6`/`kimi-k2.7-code` 不支持 `tool_choice: "required"`，降级为 `auto`；`kimi-k2.7-code` 的 `thinking` 仅接受 `{"type":"enabled","keep":"all"}`，非法配置归一化为该值。`moonshot-v1` 系列的可改参数不受影响
...
- **OpenRouter Responses 思考内容标准化** - OpenRouter 的 Responses API 实现与 OpenAI 规范存在系统性偏差：非流式 reasoning 条目把思考文本放在非标准 `content[]`（part 类型 `reasoning_text`）而规范位置 `summary[]` 留空；流式思考增量用非标准事件 `response.reasoning_text.delta/.done`、content_part 事件携带 `reasoning_text` part，而非规范的 `response.reasoning_summary_text.*` / `response.reasoning_summary_part.*` 与 `summary_text` part（分片序号也复用了 `content_index` 而非 `summary_index`）。按 OpenAI 规范实现的客户端（Codex CLI 等）只读 summary/summary 系事件，导致经 OpenRouter 中转的推理模型思考过程不可见。新增 `converters.NormalizeOpenRouterReasoningEvent/NormalizeOpenRouterReasoningResponseBody/NormalizeOpenRouterReasoningSSELine` 把非标准形态归一化为规范形态，全部按数据形状触发：仅事件名精确命中、或 summary 为空且 content 全为 reasoning_text 时才改写，标准上游零改动且幂等。接入三处：Responses 透传流式主路径 `runResponsesFold` 在逐事件解析后统一归一化（顺带让 preflight 语义判断与 thinkingcache 收集对 OpenRouter 流生效），兜底逐行中继路径 `processLine` 按行归一化（仅含 reasoning 字样的行才解析 JSON），非流式 `ConvertToResponsesResponse` 在类型化转换前归一化响应体；转 Claude 协议的 `HandleStreamResponse` 原本已双兼容两种事件名无需改动
...
- **Copilot 授权码一键复制** - channel: 为 Copilot 授权码添加一键复制操作，便于完成 OAuth 配置

### 修复

- **提示浮层不被容器裁剪** - ui: 保持 tooltip 显示在被裁剪容器之上
- **桌面端熔断器弹窗布局** - desktop: 保持熔断器弹窗布局在正常文档流中，避免内容溢出错位
- **移除全局历史图片限制控件** - ui: 移除全局 historical image limit 控件，避免与渠道级配置混淆
- **历史图片限制改为渠道级配置** - config: 将 historical image limit 调整为渠道级配置

### 重构

- **抽取 Copilot OAuth 路径常量** - channel: 提取 Copilot OAuth 路径常量并修复授权码清理逻辑

### 文档

- **澄清渠道级图片限制变更** - changelog: 澄清历史图片限制已改为渠道级配置
...
- **稳定 thinking 兼容性覆盖** - responses: stabilize thinking compatibility coverage
...
- **桌面端会话父子关系导航** - 展示会话父子关系导航，便于在关联会话间跳转
- **关联子代理会话记录** - 支持记录与展示子代理关联会话
- **桌面端会话驾驶舱任务看板** - 改造会话驾驶舱任务看板，增强子代理任务概览
- **桌面端独立驾驶舱入口** - 添加独立驾驶舱入口，提升访问效率
- **会话反馈记录** - 支持记录会话反馈信息
- **驾驶舱子代理看板改进** - 改进 cockpit subagent board 展示能力

### 修复

- **统计图表长跨度采样与展示** - 将 Web 与桌面端全局/渠道统计图默认采样点控制在 200 以内，并修复渠道级图表在仅最近几天有数据时选择 1 年导致曲线被全年空白压缩的问题
- **桌面端能力测试渠道复制反馈** - 改进能力测试渠道复制反馈体验

### 重构

- **桌面端侧边栏导航分组拆分** - 拆分侧边栏导航分组，优化导航结构
- **桌面端驾驶舱入口分离** - 将驾驶舱入口从管理控制台分离

### 文档

- **已完成 TODO 清理** - 移除已完成 TODO 项

### 其他

- **统计图表采样展示修复合并** - 合并统计图表采样展示修复
- **桌面端开发模式启动顺序** - 调整开发模式前端启动顺序
- **桌面端开发壳构建任务** - 添加开发壳构建任务
